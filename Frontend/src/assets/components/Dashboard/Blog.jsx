...
export default () => {
  ...
  const blogValues = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const res = state
        ? await axios.put("/blog/" + state?._id, formData)
        : await axios.post("/blog/add", formData);

      alert(res.data.msg);
      if (res.data.success) navigate("/dashboard/read-blog");
      console.log(res.data);
    },
  });

  const setImage = ({ target: { name, files } }) => {
    if (name === "imageURL")
      blogValues.setValues({ ...blogValues.values, imageURL: files[0] });
  };
  ...
  return (
    <Dashboard>
      <div className="w-full my-5 pt-[10px]">
        ...
        <form onSubmit={blogValues.handleSubmit}>
            ...
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-3 sm:col-span-2">
                ...
                <div className="flex gap-5 items-center">
                  ...
                  {blogValues.touched.title && blogValues.errors.title ? (
